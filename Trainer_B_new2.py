...
def main ():
    ...
    player1.DQN.train()
    player_hat.DQN.eval()
    tester = Tester(player1=Random_Agent(), player2=player1, env=env)
    #endregion
    #region ################ Wandb.init #####################
    ...
    for epoch in range(start_epoch, epochs):
        ...
        state_1.init_Board()
        action_2 = player2.get_action(state=state_1)
        is_another_turn = state_1.is_another_turn
        state_1= env.get_next_state(state=state_1, action=action_2, is_DQN = True)
        step = 0
        while not env.is_end_state(state_1):
            ...
            #calculate Q_target
            next_actions = player_hat.get_Actions(next_states, dones) # ddqn: player.get_Actions(next_states, dones)
            with torch.no_grad():
